...
class ScreenCaptureService : Service() {
    ...
    fun doScreenshot(
        outputDir: File,
        mode: RootCaptureManager.CaptureMode = RootCaptureManager.CaptureMode.NORMAL
    ) {
        scope.launch {
            _status.value = ServiceStatus.Capturing
            updateNotification(if (mode == RootCaptureManager.CaptureMode.DEEP) "深度截图…" else "正在截图…")
